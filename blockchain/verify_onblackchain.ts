...
export default async function verify_onblockchain(entry : Entry) {
  try{
    ...
    const signed_transaction = await get_sign_tx(entry_hashing.RP_id_hash, "verify_onblockchain", [entry_hashing.data_id_hash, entry_hashing.data_auth_hash.roles,  entry_hashing.data_auth_hash.goals]);
    ...
    if(signed_transaction){
      try {
        ...
        console.log(`[verify_onblockchain] data up to blockchain for data : ${entry_hashing.data_id_hash} [OK]`);
      } catch (error) {
