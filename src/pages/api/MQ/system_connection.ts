  ...
  try{
    ...
    await localChannel.consume(queue,(msg:any)=>{
      if(msg){
        ...
        await api_processing_data(msg.content.toString()); // API - processing after receiving the data
...
async function api_processing_data(msgContent:string){
  try{
    const data = JSON.parse(msgContent);
    const response = await axios.post('http://localhost:3000/api/blockchain/upto_blockchain',data);
    console.log(`API Response: ${response.Info}`) // return the result of processing
  }catch(err){
